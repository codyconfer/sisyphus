...
//
// It is the capability-aware form of ipc.IsListening: when mode.DaemonSupported is
...
// for ipc.IsListening only when a raw probe is what you want regardless of build.
func Attached(prefix, name string) bool {
	if !mode.DaemonSupported {
		return false
	}
	return ipc.IsListening(prefix, name)
}
